...
async function createScheduledJob(cron, callback) {
...
export function createBackupPlugin(BasePlugin, {
  ...
  return class ResticBackupPlugin extends BasePlugin {
    ...
    async onLoad() {
      ...
      this.unsubscribeConfig = subscribe(() => this.reloadSchedule())
      try {
        await ensure()
    ...
    async reloadSchedule() {
      const revision = this.scheduleRevision = (this.scheduleRevision ?? 0) + 1
      let nextJob
      try {
        const config = await load()
        if (this.closed || revision !== this.scheduleRevision) return
        if (config.schedule.enabled) {
          nextJob = await createJob(config.schedule.cron, () => this.scheduledBackup())
          if (!nextJob) throw new Error('定时表达式没有可执行的时间，请检查 schedule.cron')
        }
        if (this.closed || revision !== this.scheduleRevision) { nextJob?.cancel(); return }
        this.scheduledJob?.cancel()
        this.scheduledJob = nextJob
      } catch (error) {
        nextJob?.cancel()
        if (revision === this.scheduleRevision) { this.scheduledJob?.cancel(); this.scheduledJob = null }
        log('error', `更新定时任务失败：${error.message}`)
        throw error
      }
    }
    ...
    onUnload() {
      this.closed = true
      this.scheduleRevision = (this.scheduleRevision ?? 0) + 1
      this.unsubscribeConfig?.()
    ...
    async scheduledBackup() {
      try {
        ...
        if (service.state.active) { log('warn', '已有任务运行，本次定时备份跳过'); return }
        const result = await service.backup()
        log(result.partial ? 'warn' : 'info', backupResult(result))
      } catch (error) { log('error', `定时备份失败：${error.message}`) }
    }
