...
export function ossCredentials(oss, environment = process.env) {
  ...
  if ([accessKeyId, accessKeySecret, sessionToken].some(value => typeof value !== 'string' || /[\0\r\n]/.test(value))) {
    throw new Error('OSS AccessKey 和 Token 必须是单行字符串')
  }
  return { accessKeyId, accessKeySecret, sessionToken }
}
...
export async function connection(config, environment = process.env) {
  ...
  if (config.backend === 'oss') {
    ...
    const { accessKeyId, accessKeySecret, sessionToken } = ossCredentials(oss, environment)
