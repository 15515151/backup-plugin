...
export async function loadConfig({ pluginDir = PLUGIN_DIR, frameworkDir = FRAMEWORK_DIR } = {}) {
  ...
  return normalizeConfig(input, { pluginDir, frameworkDir })
}
...
export async function connection(config, environment = process.env) {
  ...
  if (config.backend === 'oss') {
    ...
    if (!/^[a-z0-9][a-z0-9-]{1,61}[a-z0-9]$/.test(oss.bucket)) throw new Error('请填写合法的 oss.bucket（3～63 位）')
    ...
    repository = `s3:${endpoint.origin}/${oss.bucket}${oss.prefix ? `/${oss.prefix}` : ''}`
    ...
    // 仓库放在备份源内会持续膨胀；同时拒绝符号链接指向源目录的情况。
    ...
    const realRepository = await canonical(repository)
